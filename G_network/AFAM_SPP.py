...
from localutils.SPPCSPC import *
...
class UNet_AFAM_attention(nn.Module):
    "在UNet加入AFAM，自适应特征注意力融合，密集跳跃连接的一种表现形式"
    def __init__(self,in_channels,out_channels):
        super(UNet_AFAM_attention,self).__init__()
        ...
        self.localBr1 = SppCSPC(32,32)
        self.localBr2  = SppCSPC(64,64)
        self.localBr3 = SppCSPC(128,128)
    ...
    def forward(self,x):
        x1 = self.localBr1(self.down_conv1(x) ) #1*32*256*256

        x2 = self.localBr2 (self.down_conv2(x1) ) #1*64*128*128

        x3 = self.localBr3 (self.down_conv3(x2) )   #1*128*64*64
