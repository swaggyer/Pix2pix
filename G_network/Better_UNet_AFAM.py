...
class UNet_AFAMSE(nn.Module):
    "在UNet加入AFAM，自适应特征注意力融合，密集跳跃连接的一种表现形式"
    def __init__(self,in_channels,out_channels):
        super(UNet_AFAMSE,self).__init__()
        ...
        self.conv = nn.Sequential(nn.Conv2d(in_channels = 256,out_channels=256,kernel_size=1),
                                  nn.BatchNorm2d(256),
                                  nn.LeakyReLU(0.2))
        self.AFAM1 = AFAM_Module_SE(out_ch=128,in_chs = [32,64,128,256])
        self.AFAM2 = AFAM_Module_SE(out_ch=64,in_chs = [32,64,128,128])
        self.AFAM3 = AFAM_Module_SE(out_ch=32, in_chs=[32, 64, 128, 64])
...
class UNet_AFAMCBAM(nn.Module):
    "在UNet加入AFAM，自适应特征注意力融合，密集跳跃连接的一种表现形式"
    def __init__(self,in_channels,out_channels):
        super(UNet_AFAMCBAM,self).__init__()
        # self.down_conv1 = CACB_Module(in_ch=in_channels,out_ch=32,last=False)
        self.down_conv1 = _ConvBNReLU(in_channels=in_channels,out_channels=32,padding=1)
        self.down_conv2 = _DSConv(dw_channels=32,out_channels=64)
        self.down_conv3 = _DSConv(dw_channels=64,out_channels=128)
        ...
        self.conv = nn.Sequential(nn.Conv2d(in_channels = 256,out_channels=256,kernel_size=1),
                                  nn.BatchNorm2d(256),
                                  nn.LeakyReLU(0.2))
        self.AFAM1 = AFAM_Module_CBAM(out_ch=128,in_chs = [32,64,128,256])
        self.AFAM2 = AFAM_Module_CBAM(out_ch=64,in_chs = [32,64,128,128])
        self.AFAM3 = AFAM_Module_CBAM(out_ch=32, in_chs=[32, 64, 128, 64])
        self.lastconv = CACB_Module(in_ch=32,out_ch=out_channels,last=True)
...
class UNet_AFAMECA(nn.Module):
    "在UNet加入AFAM，自适应特征注意力融合，密集跳跃连接的一种表现形式"
    def __init__(self,in_channels,out_channels):
        super(UNet_AFAMECA,self).__init__()
        # self.down_conv1 = CACB_Module(in_ch=in_channels,out_ch=32,last=False)
        self.down_conv1 = _ConvBNReLU(in_channels=in_channels,out_channels=32,padding=1)
        self.down_conv2 = _DSConv(dw_channels=32,out_channels=64)
        self.down_conv3 = _DSConv(dw_channels=64,out_channels=128)
